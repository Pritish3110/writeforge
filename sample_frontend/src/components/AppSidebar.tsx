...
import {
  ...
  FlaskConical, PenTool, Settings, Moon, Sun, BookOpen, Rocket, Wrench,
  ...
  const handleSignOut = async () => {
    ...
    try {
      ...
      toast.success("Signed out", {
        description: "Your local cache was cleared safely and your session has ended.",
      });
    } catch (error) {
                ...
                <div className="rounded-[12px] border border-border bg-card px-4 py-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-sm">{userDisplayName ?? userEmail ?? "Active account"}</p>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {emailVerified ? "Verified Supabase identity." : "Verification pending."}
                      </p>
                    </div>
                  </div>
                </div>
