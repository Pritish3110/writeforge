...
import { WRITEFORGE_WORLD_ELEMENTS_KEY } from "@/lib/storageKeys";
...
const WorldElementDesigner = ({ showIntro = true }: { showIntro?: boolean }) => {
  ...
  const [storedWorldElements, setStoredWorldElements] = useLocalStorage<unknown[]>(
  ...
  const savedElements = useMemo(
    () =>
      Array.isArray(storedWorldElements)
        ? storedWorldElements
  ...
  const handleGeneratePrompt = () => {
    const result = generateWorldElementPrompt({ category: selectedCategory });
  ...
  const handleSave = () => {
    ...
    setStoredWorldElements((prev) => {
      const currentRecords = Array.isArray(prev)
        ? prev.map((record, index) => normalizeWorldElementRecord(record, index))
        : [];
      return sortWorldElements([
        nextRecord,
        ...currentRecords.filter((record) => record.id !== nextRecord.id),
      ]);
    });
  ...
  const handleDelete = async (record: WorldElementRecord) => {
    const shouldDelete = await confirmDelete({
      ...
      description: "This saved world element will be removed from your workspace library. This action cannot be undone.",
    ...
    setStoredWorldElements((prev) => {
      const currentRecords = Array.isArray(prev)
        ? prev.map((entry, index) => normalizeWorldElementRecord(entry, index))
        : [];
      return currentRecords.filter((entry) => entry.id !== record.id);
    });
    ...
    <div className="space-y-4 rounded-xl border border-border bg-muted/20 p-4">
        ...
        {showIntro ? (
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Build one story-usable world element with structured notes, prompt support, and synced storage for later reference.
            </p>
          </div>
        ) : (
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Build reusable world elements, save them across your workspace, and pass them into Scene Practice when you want to write with them.
            </p>
          </div>
        )}
      ...
      <div className="rounded-lg border border-border bg-background/30 p-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="font-mono text-xs uppercase tracking-wider text-muted-foreground">Prompt Sync</p>
            <p className="mt-1 text-xs text-muted-foreground">
              When unlocked, Generate Prompt fills the generated title, category, element, and section breakdown. When locked, it only syncs the title, category, and element.
            </p>
          </div>
        ...
        <p className="mt-3 font-mono text-[11px] uppercase tracking-[0.16em] text-muted-foreground">
          {lockPromptSync ? "UI locked from prompt sync" : "UI follows the latest generated prompt"}
        </p>
      </div>
        ...
        <div className="rounded-lg border border-border bg-background/40 p-4">
          ...
          {lockPromptSync && (
            <p className="mt-3 text-xs text-muted-foreground">
              Prompt sync is locked, so only the generated title, category, and element were applied. The section fields were left untouched.
            </p>
          )}
        </div>
      )}
      ...
      <div className="space-y-3 border-t border-border pt-4">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Saved elements stay attached to your workspace, grouped by category and ready for future Lore Library integration.
          </p>
        </div>
