...
const DailyTasks = () => {
  ...
  const dayTasks = getDailyTasksForDay(selectedDay, tasks);
  const completed = dayTasks.filter((task) => isCompleted(task.id)).length;
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        {!isViewingToday && (
          <p className="text-xs text-muted-foreground mt-2 font-mono">
            Checked tasks stay with this week. Show up on the right day to keep the streak alive.
          </p>
        )}
      </div>
      ...
      <div className="space-y-1">
        <div className="flex justify-between text-sm font-mono text-muted-foreground">
          <span>{completed}/{dayTasks.length} completed</span>
          <span>{pct}%</span>
        </div>
