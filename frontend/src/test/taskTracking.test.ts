...
import { syncCustomTasks } from "@/lib/customTasks";
import {
  getCategoryStats,
...
describe("taskTracking", () => {
  ...
  it("uses edited default daily tasks when building the weekly schedule", () => {
    const taskLibrary = syncCustomTasks([
      {
        id: "mon-1",
        title: "Character Pressure",
        category: "Character",
        durationMinutes: 15,
        prompt: "Rewrite the core character drill with tighter pressure.",
        steps: [{ id: "mon-1-step-1", text: "Push the contradiction harder." }],
        importantRules: [{ id: "mon-1-rule-1", text: "Keep the desire concrete.", enabled: true }],
        writingPrinciples: ["Pressure reveals character."],
        review: ["Did the contradiction create movement?"],
        assignedDay: "Tuesday",
    ...
    const week = getCurrentWeekSummary([], taskLibrary, localDate(2026, 4, 6));
    const monday = week.find((day) => day.day === "Monday");
    const tuesday = week.find((day) => day.day === "Tuesday");

    expect(monday?.tasks.some((task) => task.id === "mon-1")).toBe(false);
    expect(
      tuesday?.tasks.some((task) => task.id === "mon-1" && task.title === "Character Pressure"),
    ).toBe(true);
  });
