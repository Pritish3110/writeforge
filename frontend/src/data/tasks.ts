...
export const PROMPT_TONES: PromptTone[] = [
  "dark",
  "emotional",
  "action",
  "mystery",
  ...
  romantic: [
    {
      id: "romantic_001",
      template:
        "{character} shares a brief, charged moment at {location} during the {phase} phase, but {conflict} keeps them from saying what they mean. They must decide whether to {choice} before {stakes}. Let the chemistry glow with {emotion}.",
      tags: ["romantic", "chemistry", "choice"],
    },
    {
      id: "romantic_002",
      template:
        "Set the scene in {location}. During the {phase} phase, {character} thinks they can keep things casual until {twist}. Make the scene turn on {choice}, with {traitHint} shaping every pause.",
      tags: ["romantic", "restraint", "turn"],
    },
    {
      id: "romantic_003",
      template:
        "{character} arrives at {location} expecting an ordinary exchange, but {conflict} makes it intimate. Treat the beat as a {phase} scene where {stakes}. Let the tenderness sharpen because {innerPressure}.",
      tags: ["romantic", "intimate", "stakes"],
    },
    {
      id: "romantic_004",
      template:
        "Write a scene where {character} and someone they want badly are trapped together at {location}. During the {phase} phase, the central question is whether they will {choice} even though {conflict}. The mood should carry {emotion}.",
      tags: ["romantic", "close-quarters", "desire"],
    },
    {
      id: "romantic_005",
      template:
        "At {location}, {character} notices a tiny gesture that changes how they read {conflict}. Frame this as a {phase} beat and let the turn come when {twist}.",
      tags: ["romantic", "gesture", "realization"],
    },
    {
      id: "romantic_006",
      template:
        "{character} is supposed to leave {location}, but the conversation about {conflict} keeps pulling them back. Use the {phase} phase to build toward a choice: {choice}. Let {traitHint} shape the romantic tension.",
      tags: ["romantic", "pull-back", "tension"],
    },
  ],
  horror: [
    {
      id: "horror_001",
      template:
        "{character} reaches {location} during the {phase} phase and realizes {conflict}. They must decide whether to {choice} before {stakes}. Let the scene throb with {emotion}.",
      tags: ["horror", "realization", "survival"],
    },
    {
      id: "horror_002",
      template:
        "Write a horror scene in {location}. During the {phase} phase, {character} senses {twist}, but the real terror comes from {conflict}. Keep the dread close because {traitHint}.",
      tags: ["horror", "dread", "wrongness"],
    },
    {
      id: "horror_003",
      template:
        "At {location}, {character} notices one small wrongness linked to {conflict}. Treat it as a {phase} beat where every attempt to stay calm makes {stakes} more likely. Let {innerPressure} erode their judgment.",
      tags: ["horror", "wrongness", "erosion"],
    },
    {
      id: "horror_004",
      template:
        "{character} thought they understood the danger inside {location}, but {twist}. During the {phase} phase, force them to choose whether to {choice} while {emotion} crawls through the scene.",
      tags: ["horror", "reversal", "panic"],
    },
    {
      id: "horror_005",
      template:
        "Set the scene in {location}. During the {phase} phase, {character} must move deeper into the place even though {conflict}. The scene should tighten around the cost of {choice}.",
      tags: ["horror", "descent", "cost"],
    },
    {
      id: "horror_006",
      template:
        "Write a scene where {character} cannot tell whether the threat at {location} is physical, supernatural, or inside them. Frame it as a {phase} beat shaped by {conflict}, ending when {twist}.",
      tags: ["horror", "uncanny", "ambiguity"],
    },
  ],
  hopeful: [
    {
      id: "hopeful_001",
      template:
        "{character} arrives at {location} during the {phase} phase carrying {conflict}, but this scene should ask whether they can {choice} before {stakes}. Let the emotional current feel like {emotion}.",
      tags: ["hopeful", "repair", "choice"],
    },
    {
      id: "hopeful_002",
      template:
        "Set the scene in {location}. During the {phase} phase, {character} sees the first believable sign that things could improve, even though {conflict}. Make the turn come when {twist}.",
      tags: ["hopeful", "sign", "change"],
    },
    {
      id: "hopeful_003",
      template:
        "Write a hopeful scene where {character} takes one concrete step toward repair at {location}. Frame it as a {phase} beat in which {choice} matters because {stakes}. Let {traitHint} make the optimism feel earned.",
      tags: ["hopeful", "repair", "earned"],
    },
    {
      id: "hopeful_004",
      template:
        "At {location}, {character} has to decide whether to trust the possibility of change. During the {phase} phase, build the scene around {conflict}, {emotion}, and {innerPressure}.",
      tags: ["hopeful", "trust", "possibility"],
    },
    {
      id: "hopeful_005",
      template:
        "{character} expects disappointment at {location}, but instead finds {twist}. Treat the scene as a {phase} moment that opens a path through {conflict} if they are willing to {choice}.",
      tags: ["hopeful", "surprise", "path"],
    },
    {
      id: "hopeful_006",
      template:
        "During the {phase} phase, {character} shares a quiet victory at {location}. The celebration is fragile because {conflict}, and the scene should matter because {stakes}.",
      tags: ["hopeful", "quiet-victory", "fragile"],
    },
  ],
  comedic: [
    {
      id: "comedic_001",
      template:
        "{character} walks into {location} during the {phase} phase and immediately collides with {conflict}. They must {choice} before {stakes}, and the scene should stay buoyant with {emotion}.",
      tags: ["comedic", "collision", "buoyant"],
    },
    {
      id: "comedic_002",
      template:
        "Write a comedic scene at {location} where {character} tries to stay in control, but {twist}. Treat it as a {phase} beat powered by timing, embarrassment, and {traitHint}.",
      tags: ["comedic", "timing", "embarrassment"],
    },
    {
      id: "comedic_003",
      template:
        "At {location}, {character} thinks one small lie will solve {conflict}. Instead, the lie escalates until they have to {choice}. Let {innerPressure} make the scene funnier and messier.",
      tags: ["comedic", "lie", "escalation"],
    },
    {
      id: "comedic_004",
      template:
        "Set the scene in {location}. During the {phase} phase, {character} is forced to juggle {conflict} while protecting something ridiculous from collapse. The scene should turn on whether they will {choice} before {stakes}.",
      tags: ["comedic", "juggle", "collapse"],
    },
    {
      id: "comedic_005",
      template:
        "{character} reaches {location} expecting relief, but {twist}. Frame the scene as a {phase} moment where every attempt to fix {conflict} makes the situation more absurd.",
      tags: ["comedic", "absurd", "relief"],
    },
    {
      id: "comedic_006",
      template:
        "Write a fast, comic pressure-cooker scene in {location}. During the {phase} phase, {character} must decide whether to {choice}, even though {conflict}. Keep the energy bright, chaotic, and rooted in {emotion}.",
      tags: ["comedic", "pressure-cooker", "chaotic"],
    },
  ],
};
...
export const PROMPT_CONFLICTS: Record<PromptTone, string[]> = {
  ...
  romantic: [
    "an attraction complicated by timing neither of them can control",
    "the fear that saying it aloud will change everything",
    "a history of almost-moments neither person has forgotten",
    "chemistry that collides with loyalty to someone else",
    "the suspicion that they want different futures",
    "the risk of being cherished by the wrong person at the wrong time",
  ],
  horror: [
    "a sound behind the walls that answers their breathing",
    "missing time no one else remembers losing",
    "the feeling that their own body is no longer fully theirs",
    "proof that the person they love may have come back wrong",
    "a place that seems to be learning their name",
    "the certainty that the threat only becomes real when acknowledged",
  ],
  hopeful: [
    "the first apology that feels honest enough to answer",
    "a fragile truce that could become something better",
    "the chance to rebuild after everyone assumed it was over",
    "proof that one act of kindness mattered more than they knew",
    "a community deciding to choose one another again",
    "evidence that an old wound may finally be able to heal",
  ],
  comedic: [
    "a mistaken identity that keeps snowballing",
    "a simple plan becoming absurdly overcomplicated",
    "a secret that is impossible to keep for five more minutes",
    "a public event drifting toward avoidable disaster",
    "a rival who enjoys the chaos far too much",
    "a well-meant lie breeding three new problems on contact",
  ],
};
...
export const PROMPT_LOCATIONS: Record<PromptTone, string[]> = {
  ...
  romantic: [
    "a rooftop garden after closing time",
    "a library aisle while rain taps the windows",
    "a late-night diner with only one booth still occupied",
    "a train car just before the final stop",
    "a community center after the dance class ends",
    "a florist cooler that still smells like spring",
  ],
  horror: [
    "a drained swimming pool at midnight",
    "a farmhouse hallway lined with wet footprints",
    "a motel room with a humming television",
    "a storm cellar beneath a silent house",
    "a children's ward after visiting hours",
    "an elevator that stops on the wrong floor",
  ],
  hopeful: [
    "a community garden after a hard winter",
    "a school gym during neighborhood cleanup",
    "a sunrise bus stop where people keep making room",
    "a repair shop reopening for the first time in months",
    "a kitchen during a volunteer meal service",
    "an apartment roof just after the power comes back",
  ],
  comedic: [
    "a disastrous wedding rehearsal",
    "an office kitchen ten minutes before a presentation",
    "a crowded costume shop on deadline",
    "a family restaurant during the lunch rush",
    "a dog park during a local news segment",
    "an apartment hallway in the middle of a fire drill",
  ],
};
...
export const PROMPT_EMOTIONS: Record<PromptTone, string[]> = {
  ...
  romantic: ["charged tenderness", "nervous longing", "playful desire", "guarded warmth", "aching attraction", "breathless hope"],
  horror: ["crawling dread", "primal panic", "sick disbelief", "helpless vigilance", "airless terror", "numb revulsion"],
  hopeful: ["cautious optimism", "hard-won relief", "gentle courage", "renewed trust", "shared warmth", "quiet joy"],
  comedic: ["secondhand embarrassment", "frazzled delight", "chaotic confidence", "mock outrage", "bright panic", "disbelieving amusement"],
};
...
export const PROMPT_STAKES: Record<PromptTone, string[]> = {
  ...
  romantic: [
    "the chance to be honest with each other will close again",
    "they will mistake fear for incompatibility",
    "the moment will pass and harden into regret",
    "someone else will define the relationship for them",
    "their silence will teach both of them the wrong lesson",
    "the connection will remain only hypothetical",
  ],
  horror: [
    "they will let the threat fully inside",
    "they will trap someone else in the same nightmare",
    "the next door they open will not lead back out",
    "their mind will stop distinguishing warning from hallucination",
    "the thing hunting them will learn how to sound human",
    "survival will require becoming complicit in the horror",
  ],
  hopeful: [
    "the fragile repair between them will collapse before it starts",
    "the new path will close if nobody acts now",
    "they will retreat back into the version of themselves that gave up",
    "the people depending on this small win will lose heart",
    "the proof of change will go unused",
    "the future they can almost imagine will slip away again",
  ],
  comedic: [
    "the misunderstanding will become public in the worst possible way",
    "the person they are trying to impress will witness the collapse live",
    "the lie will expand beyond anything fixable with dignity",
    "the entire event will derail because of one tiny mistake",
    "someone innocent will get blamed for the chaos",
    "they will have to explain everything while still covered in the evidence",
  ],
};
...
export const PROMPT_TWISTS: Record<PromptTone, string[]> = {
  ...
  romantic: [
    "the other person was trying to confess the same thing",
    "the interruption they feared creates unexpected privacy",
    "a gesture they misread was deliberate all along",
    "the person leaving had already chosen to stay",
    "the keepsake in the scene was meant for them",
    "what sounded like rejection is really fear of wanting too much",
  ],
  horror: [
    "the locked exit was never there when they entered",
    "the voice calling for help is using a memory only they know",
    "the safe object in the room has already been touched by the threat",
    "the person they came to save is calmly waiting in the wrong place",
    "the marks on the wall match something from their own body",
    "the thing stalking them wants permission, not blood",
  ],
  hopeful: [
    "someone they thought they had lost faith in shows up ready to help",
    "the damage is real but not final",
    "the person they feared hurting already took the first step toward repair",
    "the small effort they dismissed has quietly changed other people too",
    "what looked like an ending is actually a beginning they can use",
    "the room they expected to be empty has been prepared for welcome",
  ],
  comedic: [
    "the person they were trying to avoid is running the event",
    "the ridiculous prop turns out to be essential",
    "everyone else already knows the secret except the person who should",
    "the fake solution works so well it creates a worse problem",
    "the authority figure is even less composed than they are",
    "the only witness thinks the whole disaster is intentional performance art",
  ],
};
...
export const PROMPT_CHOICES: Record<PromptTone, string[]> = {
  ...
  romantic: [
    "admit the feeling before the moment cools",
    "stay long enough to risk being understood",
    "drop the practiced joke and speak plainly",
    "take one step closer instead of retreating",
    "ask the question they have been avoiding",
    "let tenderness show before fear edits it away",
  ],
  horror: [
    "go toward the sound instead of barricading the door",
    "touch the evidence nobody else wants near them",
    "wake the sleeping person even if it means spreading the terror",
    "keep moving despite the certainty that something is following",
    "destroy the object that might also be their only protection",
    "say out loud what they think is inside the room",
  ],
  hopeful: [
    "accept help instead of performing self-sufficiency",
    "say yes to the repaired future in front of them",
    "trust the apology enough to answer it honestly",
    "offer the first piece of practical hope",
    "stay present through the awkward beginning of healing",
    "protect the small good thing before cynicism kills it",
  ],
  comedic: [
    "commit to the bit and hope they can recover later",
    "confess the mistake before the cover story mutates again",
    "improvise in public with no good options left",
    "rope the nearest bystander into helping",
    "double down for thirty more impossible seconds",
    "turn the embarrassment into momentum before it turns into disaster",
  ],
};
