...
const WorldElementDesigner = ({ showIntro = true }: { showIntro?: boolean }) => {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(WORLD_ELEMENTS_STORAGE_KEY, JSON.stringify(savedElements));
    } catch {
      // Ignore local storage write failures so the editor remains usable.
    }
  }, [savedElements]);
  ...
  const handleDelete = async (record: WorldElementRecord) => {
    const shouldDelete = await confirmDelete({
      ...
      description: "This saved world element will be removed from your local library. This action cannot be undone.",
        ...
        {showIntro ? (
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Build one story-usable world element with structured notes, prompt support, and local storage for later reference.
            </p>
          </div>
        ) : (
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Build reusable world elements, save them locally, and pass them into Scene Practice when you want to write with them.
            </p>
          </div>
        )}
